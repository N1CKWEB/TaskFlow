...
def get_or_create_personal_project(user_id):
    ...
    Si no existe, lo crea y lo asocia como LIDER.
    """
    conn = get_connection()
    cur = conn.cursor(dictionary=True)
    try:
        # ¿ya tiene proyecto personal?
        cur.execute("""
          SELECT p.id_proyecto
          FROM proyectos p
          JOIN proyecto_miembros pm ON pm.id_proyecto=p.id_proyecto
          JOIN roles r ON r.id_rol=pm.id_rol
          WHERE p.creado_por=%s AND pm.id_usuario=%s AND r.codigo='LIDER'
...
def user_is_leader_of_task(user_id, id_tarea):
    """Verifica si el usuario es LIDER del proyecto dueño de la tarea."""
    conn = get_connection()
    cur = conn.cursor(dictionary=True)
    try:
        cur.execute("""
          ...
          JOIN roles r ON r.id_rol = pm.id_rol
          WHERE t.id_tarea=%s AND pm.id_usuario=%s
          LIMIT 1
        """, (id_tarea, user_id))
        row = cur.fetchone()
        return row and row['codigo'] == 'LIDER'
    finally:
        cur.close(); conn.close()
